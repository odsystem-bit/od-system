...
// ──────────────────────────────────────────────
//  Actions (removed — now in Show page)
// ──────────────────────────────────────────────
</script>

<template>
    <Head title="Mes commandes Studios" />
        ...
        <div class="mx-auto max-w-5xl px-4 py-6 sm:px-6 space-y-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">Vos commandes de contenu video aupres des createurs de contenu.</p>
                </div>
                <Link
                    :href="route('vendor.studios.index')"
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
                                <span class="flex items-center gap-1">
                                    ...
                                    <Link v-if="order.influencer" :href="route('vendor.influencer.show', order.influencer.id)" class="font-medium text-purple-600 hover:text-purple-800 underline">{{ order.influencer.name }}</Link>
                                    <template v-else>—</template>
                                </span>
                                <span v-if="order.service?.type" class="flex items-center gap-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" d="M15.75 10.5l4.72-4.72a.75.75 0 011.28.53v11.38a.75.75 0 01-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 002.25-2.25v-9a2.25 2.25 0 00-2.25-2.25h-9A2.25 2.25 0 002.25 7.5v9a2.25 2.25 0 002.25 2.25z" />
                                    </svg>
                                    {{ typeLabels[order.service.type] || order.service.type }}
                                </span>
                                ...
                                <span class="font-semibold text-slate-900">{{ formatCurrency(order.amount) }}</span>
                            </div>

                            <!-- Brief (apercu) -->
                            <p class="mt-2 text-xs text-slate-500 line-clamp-2">{{ order.brief }}</p>
                        </div>

                        <!-- Lien vers le detail -->
                        <div class="flex shrink-0 items-start">
                            <Link
                                :href="route('vendor.service-orders.show', order.id)"
            ...
            <div v-else class="rounded-xl border border-dashed border-slate-300 bg-white p-12 text-center">
                ...
                <p class="mt-1.5 text-sm text-slate-500">
                    Commandez du contenu video aupres des createurs de contenu VIP.
                </p>
                <Link
                    :href="route('vendor.studios.index')"
