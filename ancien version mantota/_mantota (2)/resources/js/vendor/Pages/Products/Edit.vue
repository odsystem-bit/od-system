...
/**
 * Props transmises par Vendor\ProductController@edit.
 * - product : Objet Product a editer.
 */
...
// ──────────────────────────────────────────────
//  Form — pre-rempli avec les donnees existantes
// ──────────────────────────────────────────────
...
const form = useForm({
    ...
    commission_percent: props.product.commission_percent ?? '',
    stock: props.product.stock ?? '',
    delivery_type: props.product.delivery_type ?? '',
    delivery_fee: props.product.delivery_fee ?? '',
    access_url: props.product.access_url ?? '',
    images: [],
    removed_image_ids: [],
    image_order: [],
});
...
// existing = { id, url } from server | new = { file, url } blobs
...
function addFiles(files) {
    ...
    for (const file of files) {
        if (totalImages.value + newImages.value.length >= MAX_IMAGES) break;
...
function removeExistingImage(index) {
    ...
    if (removed?.id) form.removed_image_ids.push(removed.id);
    syncForm();
}
...
function moveNew(from, to) {
    if (to < 0 || to >= newImages.value.length) return;
    const item = newImages.value.splice(from, 1)[0];
    newImages.value.splice(to, 0, item);
    syncForm();
}

function syncForm() {
    form.images = newImages.value.map(p => p.file);
    form.image_order = existingImages.value.map(img => img.id);
}

// ──────────────────────────────────────────────
//  Preview prix formate
// ──────────────────────────────────────────────
    ...
    <VendorLayout>
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-slate-900">Modifier le produit</h1>
                    <p class="mt-1 text-sm text-slate-500">Mettez a jour les informations de votre produit.</p>
                </div>
                <Link
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    <div class="mb-5">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Nature de votre produit.</p>
                    </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        >
                            ...
                            <div>
                                <span class="text-sm font-semibold" :class="form.type === 'physical' ? 'text-purple-700' : 'text-slate-900'">Produit Physique</span>
                                <p class="mt-1 text-xs leading-relaxed" :class="form.type === 'physical' ? 'text-purple-600/80' : 'text-slate-500'">Objet materiel avec gestion de stock et livraison.</p>
                            </div>
                        ...
                        >
                            ...
                            <div>
                                <span class="text-sm font-semibold" :class="form.type === 'digital' ? 'text-purple-700' : 'text-slate-900'">Produit Digital</span>
                                <p class="mt-1 text-xs leading-relaxed" :class="form.type === 'digital' ? 'text-purple-600/80' : 'text-slate-500'">Fichier, formation, lien de telechargement ou acces en ligne.</p>
                            </div>
                            <div class="absolute right-3 top-3 flex h-5 w-5 items-center justify-center rounded-full border-2 transition"
                ...
                <div v-if="form.type" class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    ...
                    <div class="space-y-5">
                        <!-- Stock (Physique) -->
                        <div v-if="isPhysical">
                            <label for="stock" class="block text-sm font-medium text-slate-700 mb-1">Quantite en stock</label>
                            <input id="stock" v-model="form.stock" type="number" min="0" step="1" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm" />
                        ...
                        <!-- Politique de livraison (Physique) -->
                        <div v-if="isPhysical" class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Politique de livraison</label>
                                <div class="space-y-2">
                                    <label class="flex items-center gap-3 rounded-lg border px-4 py-3 cursor-pointer transition" :class="form.delivery_type === 'free' ? 'border-purple-500 bg-purple-50 ring-1 ring-purple-500/20' : 'border-slate-200 hover:border-slate-300'">
                                        <input type="radio" name="delivery_type" value="free" v-model="form.delivery_type" class="h-4 w-4 border-slate-300 text-purple-600 focus:ring-purple-500" @change="form.delivery_fee = ''" />
                                        <div class="flex items-center gap-2 flex-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" :class="form.delivery_type === 'free' ? 'text-purple-600' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21 11.25v8.25a1.5 1.5 0 01-1.5 1.5H5.25a1.5 1.5 0 01-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 109.375 7.5H12m0-2.625V7.5m0-2.625A2.625 2.625 0 1114.625 7.5H12m0 0V21m-8.625-9.75h18c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125h-18c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" /></svg>
                                            <div>
                                    ...
                                    <label class="flex items-center gap-3 rounded-lg border px-4 py-3 cursor-pointer transition" :class="form.delivery_type === 'fixed' ? 'border-purple-500 bg-purple-50 ring-1 ring-purple-500/20' : 'border-slate-200 hover:border-slate-300'">
                                        <input type="radio" name="delivery_type" value="fixed" v-model="form.delivery_type" class="h-4 w-4 border-slate-300 text-purple-600 focus:ring-purple-500" />
                                        <div class="flex items-center gap-2 flex-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" :class="form.delivery_type === 'fixed' ? 'text-purple-600' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" /></svg>
                                            <div>
                                    ...
                                    <label class="flex items-center gap-3 rounded-lg border px-4 py-3 cursor-pointer transition" :class="form.delivery_type === 'pay_on_delivery' ? 'border-purple-500 bg-purple-50 ring-1 ring-purple-500/20' : 'border-slate-200 hover:border-slate-300'">
                                        <input type="radio" name="delivery_type" value="pay_on_delivery" v-model="form.delivery_type" class="h-4 w-4 border-slate-300 text-purple-600 focus:ring-purple-500" @change="form.delivery_fee = ''" />
                                        <div class="flex items-center gap-2 flex-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" :class="form.delivery_type === 'pay_on_delivery' ? 'text-purple-600' : 'text-slate-400'" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" /></svg>
                                            <div>
                            ...
                            <div v-if="form.delivery_type === 'fixed'">
                                <label for="delivery_fee" class="block text-sm font-medium text-slate-700 mb-1">Montant des frais de livraison (FCFA)</label>
                                <input id="delivery_fee" v-model="form.delivery_fee" type="number" min="0" step="50" placeholder="1000" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm" />
                        ...
                        <!-- Lien d'acces (Digital) -->
                        <div v-if="isDigital">
                            <label for="access_url" class="block text-sm font-medium text-slate-700 mb-1">Lien d'acces ou de telechargement</label>
                            <div class="relative">
                                ...
                                <input id="access_url" v-model="form.access_url" type="url" class="block w-full rounded-lg border-slate-300 pl-10 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm" />
                            </div>
                ...
                <div v-if="form.type" class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    <div class="mb-5">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Pourcentage reverse aux createurs de contenu pour chaque vente generee.</p>
                    </div>
                ...
                <div v-if="form.type" class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
                    <div class="mb-5">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Details, prix et images du produit.</p>
                    </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <input id="name" v-model="form.name" type="text" maxlength="255" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm" />
                        ...
                        <div>
                            ...
                            <textarea id="description" v-model="form.description" rows="4" maxlength="5000" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm"></textarea>
                        ...
                        <div>
                            ...
                            <input id="price" v-model="form.price" type="number" min="100" step="50" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm" />
                            <p v-if="form.errors.price" class="mt-1.5 text-sm text-red-600">{{ form.errors.price }}</p>
                        </div>

                        <!-- Images du produit — Multi-upload -->
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">
                                Images du produit
                                <span class="text-xs font-normal text-slate-400 ml-1">({{ totalImages }}/{{ MAX_IMAGES }})</span>
                            </label>

                            <!-- Images existantes -->
                            <div v-if="existingImages.length" class="mb-3">
                                <p class="text-xs font-medium text-slate-500 mb-2">Images actuelles</p>
                                <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                    ...
                                    >
                                        <img :src="img.url" alt="Image produit" class="h-full w-full object-cover" />
                                        <span v-if="idx === 0 && newImages.length === 0" class="absolute left-2 top-2 rounded-md bg-purple-600 px-1.5 py-0.5 text-[10px] font-bold text-white shadow">
                                            Couverture
                                        </span>
                                        <div class="absolute inset-x-0 bottom-0 flex items-center justify-center gap-1 bg-gradient-to-t from-black/60 to-transparent px-2 py-2 opacity-0 transition group-hover:opacity-100">
                                            <button v-if="idx > 0" type="button" @click="moveExisting(idx, idx - 1)" class="flex h-7 w-7 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow transition hover:bg-white">
                                            ...
                                            <button type="button" @click="removeExistingImage(idx)" class="flex h-7 w-7 items-center justify-center rounded-full bg-red-500 text-white shadow transition hover:bg-red-600">
                                            ...
                                            <button v-if="idx < existingImages.length - 1" type="button" @click="moveExisting(idx, idx + 1)" class="flex h-7 w-7 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow transition hover:bg-white">
                            ...
                            <div v-if="newImages.length" class="mb-3">
                                <p class="text-xs font-medium text-slate-500 mb-2">Nouvelles images</p>
                                <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                    <div
                                        v-for="(img, idx) in newImages"
                                        :key="'new-' + idx"
                                        class="group relative aspect-square overflow-hidden rounded-xl border-2 border-dashed border-purple-300 bg-purple-50/50"
                                    >
                                        <img :src="img.url" alt="Nouvelle image" class="h-full w-full object-cover" />
                                        <div class="absolute inset-x-0 bottom-0 flex items-center justify-center gap-1 bg-gradient-to-t from-black/60 to-transparent px-2 py-2 opacity-0 transition group-hover:opacity-100">
                                            <button v-if="idx > 0" type="button" @click="moveNew(idx, idx - 1)" class="flex h-7 w-7 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow transition hover:bg-white">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
                                            </button>
                                            <button type="button" @click="removeNewImage(idx)" class="flex h-7 w-7 items-center justify-center rounded-full bg-red-500 text-white shadow transition hover:bg-red-600">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                                            </button>
                                            <button v-if="idx < newImages.length - 1" type="button" @click="moveNew(idx, idx + 1)" class="flex h-7 w-7 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow transition hover:bg-white">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div
                                ...
                                :class="isDragOver ? 'border-purple-400 bg-purple-50' : 'border-slate-300 bg-slate-50 hover:border-slate-400'"
                            ...
                            >
                                ...
                                <p class="mt-3 text-sm text-slate-600">
                                    <span class="font-semibold text-purple-600">Ajouter des images</span> ou glissez-deposez ici
                                </p>
                                <p class="mt-1 text-xs text-slate-400">JPEG, PNG ou WebP — 5 Mo max — {{ MAX_IMAGES - totalImages }} restant(es)</p>
                                <input type="file" accept="image/jpeg,image/png,image/webp" multiple class="absolute inset-0 cursor-pointer opacity-0" @change="handleFilesSelected" />
                            </div>
