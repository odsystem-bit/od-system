...
exports.getVendorByShortCode = getVendorByShortCode;
exports.getVendorById = getVendorById;
exports.loadVendorProducts = loadVendorProducts;
exports.addProspectIfNotExists = addProspectIfNotExists;
exports.processMessage = processMessage;
exports.processVendorSetupComplete = processVendorSetupComplete;
exports.notifyVendorDispute = notifyVendorDispute;
const connection_1 = __importDefault(require("../db/connection"));
const axios_1 = __importDefault(require("axios"));
const billingService = __importStar(require("./billing"));
const mantotaService = __importStar(require("./mantota"));
const scannerService = __importStar(require("./scanner"));
...
(minimum 8 caractères)`,
    collect_pays: (pays) => `Je vois que tu es au ${pays} !`,
...
3️⃣ Dessert  4️⃣ Boisson`,
    boutique_created: (shopName, slug, email) => `🎉 Ta boutique ${shopName} est en ligne !
...
Tu choisis quelle offre ? (réponds 1, 2, 3 ou 4)`,
    vendor_menu: (name) => `Bon retour ${name} 👋 Que veux-tu faire ?
...
7️⃣ Mettre à jour depuis mon site`,
    list_products: (products) => {
        if (products.length === 0)
            return "Tu n'as pas encore de produits.";
...
1️⃣ Nom  2️⃣ Prix  3️⃣ Description  4️⃣ Photo`,
    ...
    view_stats: (subscription) => `📊 Stats de ce mois :
...
Abonnement expire le : ${subscription.date_fin}`,
    ...
    new_client_products_intro: (shopName) => `Super ${shopName} 🎉 Maintenant on va ajouter tes produits
un par un. C'est simple :
👉 Envoie-moi d'abord le NOM de ton premier produit`,
    collecting_product_price: (productName) => `Ok ! C'est combien le prix de ${productName} ? (en FCFA)`,
    collecting_product_description: (productName) => `Parfait ! Donne-moi une courte description de ${productName}
(2-3 lignes max)`,
    collecting_product_image: (productName) => `Tu as une photo de ${productName} ? Envoie-la moi 📸
(ou écris 'passer' si tu n'en as pas maintenant)`,
    ask_more_products: (productName) => `${productName} ajouté ✅ Tu as d'autres produits à ajouter ?
1️⃣ Oui, ajouter un autre produit
2️⃣ Non, c'est bon pour l'instant`,
    confirm_payment: (planName, price) => `Tu as choisi ${planName} à ${price} FCFA/mois 👍
...
crée ta boutique immédiatement 🚀`,
    payment_confirmed: (shopName, slug) => `Paiement confirmé ✅ Je crée ta boutique maintenant...
...
async function getOrCreateConversation(phone) {
    const [rows] = await connection_1.default.query('SELECT * FROM conversations WHERE phone_number = ?', [phone]);
    if (rows.length > 0) {
    ...
    const [result] = await connection_1.default.query('INSERT INTO conversations (phone_number, current_step, collected_data, client_type) VALUES (?, ?, ?, ?)', [phone, 'welcome', JSON.stringify({}), null]);
    return {
        id: result.insertId,
        phone_number: phone,
...
async function updateConversation(phone, step, data) {
    // Construire la requête UPDATE dynamiquement selon les clés présentes
    const updates = ['current_step = ?', 'updated_at = NOW()'];
    const values = [step];
    ...
    values.push(phone);
    await connection_1.default.query(`UPDATE conversations SET ${updates.join(', ')} WHERE phone_number = ?`, values);
}
async function resetConversation(phone) {
    await connection_1.default.query('UPDATE conversations SET current_step = ?, collected_data = ?, updated_at = NOW() WHERE phone_number = ?', ['welcome', JSON.stringify({}), phone]);
}
// Fonction pour récupérer un vendor par son code court
async function getVendorByShortCode(code) {
    const [rows] = await connection_1.default.query('SELECT * FROM vendors WHERE UPPER(short_code) = UPPER(?) AND bot_status = "active"', [code]);
    return rows.length > 0 ? rows[0] : null;
}
// Fonction pour récupérer un vendor par son ID
async function getVendorById(id) {
    const [rows] = await connection_1.default.query('SELECT * FROM vendors WHERE id = ?', [id]);
    return rows.length > 0 ? rows[0] : null;
}
// Fonction pour charger les produits d'un vendor
async function loadVendorProducts(vendorId) {
    const [rows] = await connection_1.default.query('SELECT * FROM products WHERE vendor_id = ? AND statut = "actif"', [vendorId]);
    return rows;
}
// Fonction pour ajouter un prospect s'il n'existe pas déjà
async function addProspectIfNotExists(phone, vendorId, shortCode) {
    await connection_1.default.query('INSERT INTO prospects_relance (phone_number, short_code, vendor_id, last_interaction_at) VALUES (?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE last_interaction_at = NOW()', [phone, shortCode, vendorId]);
}
async function callGPTMini(message, customSystemPrompt) {
...
async function processMessage(phone, message, messageType, imageId = null) {
    ...
    if (looksLikeCode) {
        ...
        if (targetVendor) {
            ...
            const previousVisits = history.filter((v) => v.vendor_id === targetVendor.id).length;
            if (previousVisits > 1) {
                // Client de retour
                return [
    ...
    const [vendorRows] = await connection_1.default.query('SELECT * FROM vendors WHERE phone_number = ? AND bot_status = "active"', [phone]);
    ...
    if (isVendor && conversation.client_type !== 'vendor_management') {
        ...
        await connection_1.default.query('UPDATE conversations SET client_type = ?, vendor_id = ? WHERE phone_number = ?', ['vendor_management', vendor.id, phone]);
    }
    ...
    if (isVendor && vendor) {
        ...
        if (alert) {
            ...
            const [vendorRows] = await connection_1.default.query('SELECT phone_number FROM vendors WHERE id = ?', [conversation.vendor_id]);
            if (vendorRows.length > 0) {
                const { sendMessage } = await Promise.resolve().then(() => __importStar(require('./whatsapp')));
    ...
    switch (currentStep) {
        ...
        case 'scan_site':
            ...
            try {
                ...
                let productList = '';
                scannedProducts.slice(0, 5).forEach((p, i) => {
        ...
        case 'shop_type':
            ...
            if (shopTypes[message]) {
                collectedData.shop_type = shopTypes[message];
        ...
        case 'collecting_product_category':
            ...
            if (categories[message]) {
                collectedData.current_product.categorie = categories[message];
                await updateConversation(phone, 'collecting_product_description', collectedData);
                return [MESSAGES.collecting_product_description(collectedData.current_product.nom)];
            }
            else {
        ...
        case 'choose_plan':
            ...
            if (planKeys[message]) {
                const planKey = planKeys[message];
                ...
                if (planKey === 'gratuit') {
                    ...
                    await connection_1.default.query('INSERT INTO subscriptions (vendor_id, plan, conversations_included, date_debut, date_fin) VALUES (?, ?, ?, CURDATE(), ?)', [collectedData.mantota_vendor_id, 'gratuit', 50, dateFin.toISOString().split('T')[0]]);
                    await resetConversation(phone);
        ...
        case 'confirm_payment':
            if (lowerMessage.includes('payé') || lowerMessage.includes('paye') || lowerMessage.includes('ok')) {
                ...
                const dateFin = new Date();
                dateFin.setDate(dateFin.getDate() + 30);
                await connection_1.default.query('INSERT INTO subscriptions (vendor_id, plan, conversations_included, date_debut, date_fin) VALUES (?, ?, ?, CURDATE(), ?)', [collectedData.mantota_vendor_id, collectedData.plan, PLANS[collectedData.plan].conversations, dateFin.toISOString().split('T')[0]]);
                // Si plan Pro : déclencher le flux de configuration numéro dédié
                if (collectedData.plan === 'pro') {
                    await connection_1.default.query('UPDATE vendors SET setup_status = "pending_number" WHERE id = ?', [collectedData.mantota_vendor_id]);
                ...
                await connection_1.default.query('UPDATE vendors SET bot_status = "active" WHERE id = ?', [collectedData.mantota_vendor_id]);
                await resetConversation(phone);
                return ["✅ Paiement confirmé ! Ton bot est activé. 🎉"];
            }
            else {
                const plan = PLANS[collectedData.plan];
        ...
        case 'vendor_menu':
            if (message === '1') {
                ...
                const [vendorRows] = await connection_1.default.query('SELECT mantota_vendor_id FROM vendors WHERE id = ?', [vendor.id]);
...
🏆 Top produits :
  ${stats.top_products.map((p, i) => `${i + 1}. ${p.name} — ${p.orders} ventes`).join('\n  ')}`;
        ...
        case 'modify_product_field':
            ...
            if (fields[message]) {
                collectedData.field_to_modify = fields[message];
        ...
        case 'rescan_site':
            try {
                ...
                if (compareResult.hasChanges) {
                    ...
                    await connection_1.default.query('INSERT INTO vendors_pending (phone_number, name, shop_type, scanned_products_json, status) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE scanned_products_json = ?, status = ?', [phone, vendor.name, vendor.shop_type, JSON.stringify(newProducts), 'ready', JSON.stringify(newProducts), 'ready']);
        ...
        case 'confirm_site_update':
            if (message === '1') {
                ...
                try {
                    ...
                    const [pendingRows] = await connection_1.default.query('SELECT scanned_products_json FROM vendors_pending WHERE phone_number = ? AND status = "ready"', [phone]);
                    ...
                    const [vendorRows] = await connection_1.default.query('SELECT id FROM vendors WHERE phone_number = ?', [phone]);
                    if (vendorRows.length === 0) {
                        await resetConversation(phone);
                    ...
                    await connection_1.default.query('UPDATE products SET statut = "inactif" WHERE vendor_id = ?', [vendorId]);
                    // Ajouter ou réactiver les nouveaux produits
                    for (const product of scannedProducts) {
                        // Vérifier si le produit existe déjà
                        const [existingRows] = await connection_1.default.query('SELECT id FROM products WHERE vendor_id = ? AND nom = ?', [vendorId, product.nom]);
                        if (existingRows.length > 0) {
                            // Mettre à jour le produit existant
                            await connection_1.default.query('UPDATE products SET prix = ?, description = ?, categorie = ?, statut = "actif" WHERE id = ?', [product.prix, product.description, product.categorie, existingRows[0].id]);
                        }
                        else {
                            // Créer un nouveau produit
                            await connection_1.default.query('INSERT INTO products (vendor_id, nom, prix, description, categorie, statut) VALUES (?, ?, ?, ?, ?, "actif")', [vendorId, product.nom, product.prix, product.description, product.categorie]);
                        }
                    }
                    // Nettoyer vendors_pending
                    await connection_1.default.query('DELETE FROM vendors_pending WHERE phone_number = ?', [phone]);
                    await resetConversation(phone);
        ...
        case 'pro_number_request':
            ...
            await connection_1.default.query('UPDATE vendors SET whatsapp_number = ?, setup_status = "pending_number" WHERE id = ?', [message, collectedData.mantota_vendor_id]);
            // Logger
            await logger.log('info', 'setup', `Nouveau numéro Pro reçu: ${message} pour vendor: ${collectedData.mantota_vendor_id}`);
            // Alerter l'admin
            const [vendorInfo] = await connection_1.default.query('SELECT shop_name FROM vendors WHERE id = ?', [collectedData.mantota_vendor_id]);
            const adminNumber = process.env.ADMIN_WHATSAPP_NUMBER;
            if (adminNumber) {
                const { sendMessage } = await Promise.resolve().then(() => __importStar(require('./whatsapp')));
                await sendMessage(adminNumber, `🔔 NOUVEAU NUMÉRO PRO À CONFIGURER\nBoutique : ${vendorInfo[0]?.shop_name || 'N/A'}\nNuméro : ${message}\nID vendor : ${collectedData.mantota_vendor_id}\n→ Va sur Meta Business Manager pour l'ajouter`);
            }
        ...
        case 'pro_awaiting_code':
            ...
            if (codeRegex.test(message)) {
                ...
                await connection_1.default.query('UPDATE vendors SET meta_verification_code = ?, setup_status = "pending_code" WHERE id = ?', [message, collectedData.mantota_vendor_id]);
                // Alerter l'admin
                const [vendorInfo] = await connection_1.default.query('SELECT shop_name FROM vendors WHERE id = ?', [collectedData.mantota_vendor_id]);
                const adminNumber = process.env.ADMIN_WHATSAPP_NUMBER;
                if (adminNumber) {
                    const { sendMessage } = await Promise.resolve().then(() => __importStar(require('./whatsapp')));
                    await sendMessage(adminNumber, `🔑 CODE META REÇU\nBoutique : ${vendorInfo[0]?.shop_name || 'N/A'}\nCode : ${message}\n→ Valide maintenant sur Meta Business Manager\n→ Puis clique 'Activer' dans l'admin panel`);
                }
                await resetConversation(phone);
                return [
        ...
        case 'vendor_customer_welcome':
            ...
            const productsList = products
                ...
                .map((p) => `- ${p.nom} : ${p.prix} FCFA${p.description ? ' — ' + p.description : ''}`)
                .join('\n');
...
- Quand le client exprime clairement une intention d'achat (ex: 'je veux commander', 'comment acheter', 'je prends [produit]', 'je veux ça'), réponds EXACTEMENT par ce JSON et rien d'autre : {"action":"start_order","product_mentioned":"[nom du produit mentionné ou null]"}`;
        ...
        case 'order_collect_address':
            ...
            if (collectedData.ordering_product) {
                ...
                if (orderVendorId) {
                    ...
                    const mentionedProduct = orderProducts.find((p) => p.nom.toLowerCase().includes(collectedData.ordering_product.toLowerCase()));
                    if (mentionedProduct) {
        ...
        case 'order_select_product':
            ...
            const productList = selectProducts.map((p, i) => `${i + 1}️⃣ ${p.nom} — ${p.prix} FCFA`).join('\n');
...
async function processVendorSetupComplete(vendor_id) {
    try {
        // Charger le vendor depuis DB
        const [vendorRows] = await connection_1.default.query('SELECT * FROM vendors WHERE id = ?', [vendor_id]);
        if (vendorRows.length === 0) {
        ...
        await connection_1.default.query('UPDATE vendors SET setup_status = "active", bot_status = "active" WHERE id = ?', [vendor_id]);
        // Logger l'activation
...
async function notifyVendorDispute(vendorPhone, orderRef, customerName, disputeReason) {
    try {
        const { sendMessage } = await Promise.resolve().then(() => __importStar(require('./whatsapp')));
        await sendMessage(vendorPhone, `🔴 LITIGE OUVERT\nCommande : ${orderRef}\nClient : ${customerName}\nMotif : ${disputeReason}\n\nL'argent reste bloqué jusqu'à résolution.\n➡️ Réponds depuis mantota.com/orders`);
