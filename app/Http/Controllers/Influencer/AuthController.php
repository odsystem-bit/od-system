...
/**
 * AuthController — Authentification dediee aux créateurs de contenu MANTOTA.
 *
 * Completement separe du flux admin et vendeur.
 * Seuls les comptes ayant le role UserRole::INFLUENCER peuvent se connecter.
 */
class AuthController extends Controller
{
    /**
     * Affiche le formulaire de connexion créateur de contenu.
     */
    public function showLogin(): InertiaResponse|RedirectResponse
    {
        if (Auth::guard('influencer')->check() && Auth::guard('influencer')->user()->role === UserRole::INFLUENCER) {
    ...
    /**
     * Authentifie un créateur de contenu.
     */
    public function login(Request $request): RedirectResponse
    {
        ...
        if (! Auth::guard('influencer')->attempt($request->only('email', 'password'), $request->boolean('remember'))) {
            SecurityService::log('login_failed', email: $request->input('email'), guard: 'influencer');
        ...
        if ($user->role !== UserRole::INFLUENCER) {
            ...
            throw ValidationException::withMessages([
                'email' => 'Acces refuse. Ce portail est reserve aux créateurs de contenu.',
            ]);
        }
        ...
        SecurityService::log('login_success', $user->id, $user->email, 'influencer');

        return redirect()->route('influencer.dashboard');
    }

    /**
     * Affiche le formulaire d'inscription créateur de contenu.
     */
    ...
    /**
     * Inscrit un nouveau créateur de contenu.
     */
    public function register(Request $request): RedirectResponse
    {
        ...
        // Email de bienvenue au créateur
    ...
    /**
     * Deconnecte le créateur de contenu.
     */
    ...
    public function sendResetLink(Request $request): RedirectResponse
    {
        ...
        $status = Password::broker('influencers')->sendResetLink(
            $request->only('email')
        );
    ...
    public function resetPassword(Request $request): RedirectResponse
    {
        ...
        $status = Password::broker('influencers')->reset(
        ...
        if ($status == Password::PASSWORD_RESET) {
            return redirect()->route('influencer.login')->with('status', __($status));
        }
