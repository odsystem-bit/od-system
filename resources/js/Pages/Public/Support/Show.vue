...
function submit() {
    form.post(route('support.reply', props.ticket.reference_code), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset('message');
        ...
        <header class="border-b border-slate-200 bg-white">
            <div class="mx-auto flex h-16 max-w-3xl items-center justify-between px-4 sm:px-6">
                ...
                <div class="flex items-center gap-4">
                    <a :href="route('support.track')" class="text-sm font-medium text-slate-500 hover:text-slate-700">Suivi</a>
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div class="max-h-[28rem] space-y-4 overflow-y-auto p-6">
                    <div v-for="msg in messages" :key="msg.id" :class="msg.is_admin ? 'flex justify-start' : 'flex justify-end'">
                        <div :class="msg.is_admin ? 'bg-slate-100 text-slate-800' : 'bg-teal-600 text-white'" class="max-w-[75%] rounded-2xl px-4 py-3">
                            <div class="mb-1 flex items-center gap-2">
                                <span class="text-xs font-semibold" :class="msg.is_admin ? 'text-slate-600' : 'text-teal-100'">
                                    {{ msg.is_admin ? 'Support' : (ticket.guest_name || ticket.user?.name || 'Vous') }}
                                </span>
                                <span class="text-xs" :class="msg.is_admin ? 'text-slate-400' : 'text-teal-200'">{{ formatDate(msg.created_at) }}</span>
                            </div>
